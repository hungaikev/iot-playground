...
/**
  ...
  */
class DeviceQueryGroupActorSpec extends TestKit(ActorSystem("iot-system"))
  with FlatSpecLike
  with Matchers
  ...
  "A Device Query Actor " should "return temperature value for working devices" in {

    val requester = TestProbe()
    val device1 = TestProbe()
    val device2 = TestProbe()

    val queryActor = system.actorOf(DeviceQueryGroup.props(
    ...
    requester.expectMsg(DeviceGroup.RespondAllTemperatures(requestId = 1, temperatures = Map(
  ...
  it should "return DeviceTimedOut if device does not answer in time" in {
    ...
    requester.expectMsg(DeviceGroup.RespondAllTemperatures(
      ...
      temperatures = Map(
        "device1" -> DeviceGroup.Temperature(1.0),
        "device2" -> DeviceGroup.DeviceTimeOut
      )
    ))
  }
